...
(() => {
  ...
  function render(keepScroll = false) {
    ...
    document.title = `Trov · ${{ overview: '物流总览', shipments: '包裹工作台', analytics: '物流分析', data: '数据与来源', reports: '投流报告' }[store.page]}`;
    $('#app').innerHTML = `${header()}<main id="main" class="main-container" tabindex="-1"><div class="welcome-strip"><p>欢迎回来。</p><time class="pacific-clock" data-pacific-clock aria-label="当前太平洋日期与时间"><span class="pacific-date" data-pacific-date></span><span class="pacific-time"><strong data-pacific-time></strong><small data-pacific-zone></small></span></time></div>${({ overview, shipments: shipmentsPage, analytics: analyticsPage, data: dataPage, reports: () => views.reportsPage({ catalog: REPORTS, kind: store.reportKind, query: store.reportQuery, reportId: store.reportId }, viewHelpers) })[store.page]() }${footer()}</main>${nav(true)}`;
