...
(() => {
  ...
  function weeklyChart(rows, handling = false) {
    ...
    for (let start = min; start <= snapshot; start += 7 * 86400000) {
      ...
      const group = rows.filter(s => dateValue(s.cohortAt) >= start && dateValue(s.cohortAt) <= end);
    ...
    return `<div class="chart-wrap"><div class="chart-scale"><span>${limit}</span><span>${limit / 2}</span><span>0</span></div><div class="bar-chart ${handling ? 'handling-chart' : ''}" role="img" aria-label="${handling ? '每周 OMS 仓内处理时间中位数，单位小时' : '每周 Shopify 物流单量，单位票'}">${buckets.map((b, i) => `<div class="chart-column"><div class="column-space"><div class="chart-bar ${i === buckets.length - 1 ? 'last' : ''}" style="--bar-height:${Math.max(0, b.value / limit * 100)}%" tabindex="0" aria-label="${b.label} 至 ${b.end}：${handling ? number(b.value) + ' 小时，样本 ' + b.samples : b.count + ' 票'}"><span class="bar-number">${handling ? number(b.value) : b.count}</span><span class="chart-tooltip">${b.label} – ${b.end}<br>${handling ? number(b.value) + ' 小时 · n=' + b.samples : b.count + ' 票物流单'}</span></div></div><span class="x-label">${b.label}</span></div>`).join('')}</div></div>`;
  }
  ...
  function exportCsv() {
    ...
    const columns = [['订单号', s => s.order], ['运单号', s => s.id], ['州', s => s.state], ['ZIP5', s => s.zip], ['物流状态', s => s.status || ''], ['Shopify 物流单添加时间', s => s.fulfillmentCreatedAt], ['OMS 单据状态', s => s.omsStatus], ['物流服务', s => s.service], ['费用 USD', s => s.cost], ['偏远程度', s => remoteText(s.remote)], ['OMS 创建时间', s => s.createdAt], ['OMS 发货时间', s => s.shippedAt], ['状态来源', s => s.isLive ? 'Shopify' : '历史 OMS'], ['数据读取时间', () => observedAt()]];
