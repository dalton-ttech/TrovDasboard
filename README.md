...
移动端优先的 Trov 工作区：投流总览、Shopify 物流观测、运输时效，以及独立的投流日报 / 周报。使用原生 HTML / CSS / JavaScript，无前端构建依赖。
...
**当前提交是本机应用，尚未部署到 Cloudflare。** 云端版本建议使用 Workers + Static Assets，迁移范围见 [Cloudflare 部署说明](docs/CLOUDFLARE_DEPLOYMENT.md)。

## 界面

- 顶部显示「欢迎回来。」和实时太平洋日期、时间；采用 `America/Los_Angeles`，自动切换 PDT / PST。
- 投流总览和物流总览共用标题、时间说明及日期选择样式，标题位于指标卡片外。
- 投流总览默认最近 30 个完整太平洋自然日，可切换近 7 天，截至昨日；与物流区间独立，仅显示上次读取时间。
...
「账号」页可手动读取 Shopify，报告中心可刷新归档。投流总览在服务端更新快照后重新打开页面即可看到新数据，没有前端刷新按钮。
...
自动检查覆盖语法、物流状态优先级、时效、太平洋日期与夏令时切换、有效订单和合并 ROAS。本地集成检查核对历史数据保留、报告全文字节一致、PDF 路由关闭、私有文件访问及只读调用边界。
