...
每日统一发布使用 `python -B scripts\run_daily_publish.py`，完整规范见
`docs\DASHBOARD_DAILY_PUBLISH_HEARTBEAT.md`。固定入口会补齐缺失日报、生成最近完整自然周周报、刷新销量与物流、导出、提交并验证 Pages；失败时保留上一份线上有效快照。
